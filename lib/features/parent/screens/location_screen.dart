...
class _LocationScreenState extends State<LocationScreen>
          ...
          return Stack(
            children: [
                    ...
                    markers: [
                      ...
                      Marker(
                        ...
                        child: Container(
                          ...
                          child: const Icon(Icons.house_rounded, color: Colors.greenAccent, size: 24),
                        ),
                      ),
                      Marker(
                        point: busLoc,
                ...
                Center(
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.error_outline_rounded, size: 64, color: Colors.blueAccent.withValues(alpha: 0.4)),
                        const SizedBox(height: 16),
                        Text(AppLocalizations.of(context)!.translate(vm.errorMessage!), style: TextStyle(color: isDark ? Colors.white70 : Colors.black87, fontWeight: FontWeight.w900, fontSize: 16)),
              ...
              _DraggableHistorySheet(
                history: vm.history,
                buildAlerts: () => _buildImportantAlerts(context, vm.currentLocation),
                buildStats: () => _buildBusFleetStats(context, vm.currentLocation),
    ...
    return Column(
      children: [
        GlassCard(
          ...
          color: isArrived ? Colors.greenAccent.withValues(alpha: 0.05) : Colors.orangeAccent.withValues(alpha: 0.05),
          child: Row(
            children: [
              Container(
                padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(
                    color: (isArrived ? Colors.greenAccent : Colors.orangeAccent).withValues(alpha: 0.1),
                    shape: BoxShape.circle),
                child: Icon(isArrived ? Icons.check_circle_outline_rounded : Icons.directions_bus_filled_rounded,
                    color: isArrived ? Colors.greenAccent : Colors.orangeAccent, size: 24),
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                    color: (isArrived ? Colors.greenAccent : Colors.orangeAccent).withValues(alpha: 0.2),
                    borderRadius: BorderRadius.circular(12)),
                child: Text(
                    isArrived ? "SECURED" : "LIVE",
                    style: TextStyle(
  ...
  Widget _buildFloatingStatusPills(BuildContext context, BusLocationModel? bus) {
    return Positioned(
      bottom: 120,
      left: 20,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildPill("${AppLocalizations.of(context)!.translate('gps_signal')} : ${bus != null ? 'STRONG' : 'UNKNOWN'}"),
          const SizedBox(height: 8),
          _buildPill("${bus?.batteryLevel.toInt() ?? 0}% ${AppLocalizations.of(context)!.translate('battery')}"),
        ],
      ),
    );
  }
  ...
  Widget _buildPill(String text) {
    return Container(
      ...
      child: Text(text, style: const TextStyle(color: Color(0xFF0A0F1E), fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 0.5)),
    );
  }
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildZoomBtn(icon: Icons.add_rounded, onTap: () {
                final newZoom = (_currentZoom + 1).clamp(3.0, 18.0);
                _mapController.move(_mapController.camera.center, newZoom);
                setState(() => _currentZoom = newZoom);
            }),
            Container(height: 1, width: 28, color: Colors.white.withValues(alpha: 0.08)),
            _buildZoomBtn(icon: Icons.remove_rounded, onTap: () {
                final newZoom = (_currentZoom - 1).clamp(3.0, 18.0);
                _mapController.move(_mapController.camera.center, newZoom);
                setState(() => _currentZoom = newZoom);
            }),
  ...
  Widget _buildZoomBtn({required IconData icon, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), child: Icon(icon, color: Colors.white70, size: 20)),
    );
  }
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: Colors.blueAccent.withValues(alpha: 0.2), blurRadius: 8)],
          ),
          child: Text(bus?.id ?? "BUS", style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900)),
        ),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: Colors.blueAccent.withValues(alpha: 0.4), blurRadius: 15, spreadRadius: 5)],
          ),
  ...
  Widget _buildTimelineItem(BuildContext context, LocationHistoryRecord record) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(color: record.color.withValues(alpha: 0.15), shape: BoxShape.circle),
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text(record.title, style: TextStyle(color: isDark ? Colors.white : const Color(0xFF0F172A), fontWeight: FontWeight.w900, fontSize: 15)),
                    Text(record.time, style: TextStyle(color: isDark ? Colors.white38 : Colors.black38, fontSize: 12, fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 4),
                Text(record.subtitle, style: TextStyle(color: isDark ? Colors.white60 : Colors.black45, fontSize: 13, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
        ],
