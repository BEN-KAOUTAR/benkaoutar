...
class _TeacherChatScreenState extends State<TeacherChatScreen>
      ...
      body: DeepSpaceBackground(
        ...
        child: SafeArea(
          child: TabBarView(
            controller: _tabController,
            children: [
              _buildMessagesTab(context, classes, isDark, primaryTextColor,
                  secondaryTextColor),
              _buildGroupsTab(context, classes, isDark, primaryTextColor,
                  secondaryTextColor),
            ],
          ),
        ),
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          onPressed: () => Navigator.push(context,
              MaterialPageRoute(builder: (_) => const NewChatSearchScreen())),
  ...
  Widget _buildMessagesTab(BuildContext context, List<dynamic> classes,
    ...
    return ListView(
      ...
      children: [
        if (classes.isNotEmpty && classes.first.students.length >= 4) ...[
          _ChatTile(
            name:
                '${loc.translate('parent_of')} ${classes.first.students[0].name}',
          ...
          _ChatTile(
            name:
                '${loc.translate('parent_of')} ${classes.first.students[2].name}',
  ...
  Widget _buildGroupsTab(BuildContext context, List<dynamic> classes,
      bool isDark, Color primaryTextColor, Color secondaryTextColor) {
    ...
    return Container(
      ...
      child: ListTile(
        onTap: () => Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => ChatDetailScreen(
                name: name, avatarUrl: 'https://i.pravatar.cc/150?u=$name'),
          ),
        ),
...
class _NewChatSearchScreenState extends State<NewChatSearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredParents =
        _parentNames.where((n) => n.toLowerCase().contains(_query)).toList();
    final filteredClasses = <ClassModel>[]
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSettingsItem(Icons.info_outline_rounded,
                loc.translate('voir_details'), Colors.blueAccent, () {
              ...
              if (widget.classModel != null) {
                ...
                final allStudents =
                    <ClassModel>[].expand((c) => c.students).toList();
                final student = allStudents.firstWhere(
                    (s) => s.name.contains(widget.name.split(' ').last),
                    orElse: () => allStudents.first);
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (_) =>
                            StudentDetailFullScreen(student: student)));
              }
            }),
            _buildSettingsItem(
            ...
            _buildSettingsItem(Icons.delete_outline_rounded,
                loc.translate('clear_chat'), Colors.redAccent, () {
              Navigator.pop(context);
              _confirmDeleteAll();
            }),
            const SizedBox(height: 24),
            ...
            GridView.count(
              ...
              children: [
                ...
                _buildAttachmentIcon(
                    ...
                    loc.translate('selfie'),
                    Colors.blueAccent,
                    () => _simulateMediaPicker(context, loc.translate('selfie'),
                        Icons.camera_alt_rounded)),
                _buildAttachmentIcon(
                    Icons.video_collection_rounded,
                    loc.translate('video_label'),
                    Colors.orangeAccent,
                    () => _simulateMediaPicker(
                        context,
                        loc.translate('video_label'),
                ...
                _buildAttachmentIcon(
                    Icons.link_rounded,
                    loc.translate('link_label'),
                    Colors.tealAccent,
                    () => _simulateMediaPicker(context,
                        loc.translate('link_label'), Icons.link_rounded)),
                _buildAttachmentIcon(
                    Icons.picture_as_pdf_rounded,
                    loc.translate('pdf_label'),
                    Colors.redAccent,
                    () => _simulateMediaPicker(
                        context,
                        loc.translate('pdf_label'),
                ...
                _buildAttachmentIcon(
                    Icons.mic_rounded,
                    loc.translate('audio_recording'),
                    Colors.greenAccent,
                    () => _simulateMediaPicker(context,
                        loc.translate('audio_recording'), Icons.mic_rounded)),
              ],
            ),
            const SizedBox(height: 32),
          ],
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      _sendAttachment(
                          (type.contains('Photo') ||
                                  type.contains('Galerie') ||
                                  type.contains('Selfie'))
                              ? 'photo'
                              : (type.contains('Audio') ? 'audio' : 'text'),
                          (type.contains('Photo') ||
                                  type.contains('Galerie') ||
                                  type.contains('Selfie'))
                              ? ''
        ...
        child: Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              boxShadow:
                  isMe ? [] : [BoxShadow(color: Colors.white, blurRadius: 10)],
              border: isSelected
              ...
              GestureDetector(
                ...
                onLongPressEnd: isAdminOnly
                    ...
                    : (_) {
                        ...
                        _stopRecordingTimer();
                        _sendAttachment('audio', 'Voice note',
                            duration: duration);
                      },
